function [data,hdr]=cbiReadNifti_unix(fname,subset,prec,short_nan)
...
if isempty(a)
    ...
    fileunzip = [a '\' base '.nii'];
    filezip = [a '\' base '.nii.gz'];
end;

if isequal(exist(fileunzip,'file'),2)
    if isequal(exist(filezip,'file'),2)
        warning('Both the .nii and .nii.gz file exists, reading the .gz file');
    end;
end;
...
hdr=cbiReadNiftiHeader_unix(fname);
