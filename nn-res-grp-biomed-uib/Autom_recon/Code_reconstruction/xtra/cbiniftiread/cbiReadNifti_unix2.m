function [data,hdr]=cbiReadNifti_unix2(fname,subset,prec,short_nan)
...
% test if both files exist and print a warning
[a b ext1] = fileparts(fname);
[e base ext2] = fileparts(b);
if isempty(a)
    fileunzip = [pwd base '.nii'];
    filezip = [pwd base '.nii.gz'];
else
    fileunzip = [pwd a '\' base '.nii'];
    filezip = [pwd a '\' base '.nii.gz'];
end;

if isequal(exist(fileunzip,'file'),2)
    if isequal(exist(filezip,'file'),2)
        warning('Both the .nii and .nii.gz file exists, reading the .nii file');
    end;
end;

% the zip file exists, then use that one
%unzip = 1;
unzip = 0;
if isequal(exist(filezip,'file'),2)    
    unzip = 1;        
    fname = unzip_temp_unix(filezip);
end;

% % if isequal(exist(fname,'file'),2) && exist([fname '.gz'] ,'file'),2)
% if ~isequal(exist(fname,'file'),2)  
%     'dfd'
%     if isequal(exist([fname '.gz'],'file'),2)
%         
%         % unzip into memory to save time!!
%         fname = unzip_temp('segmgroup_Putamen_blob.nii.gz');
% %         % unzip to file and keep the original file, so we dont need to zip
% %         % again afterwards
% %         cmd = ['gunzip -f -c ' fname '.gz' ' >> ' fname];
% %         system(cmd);
% %         flag = 1;
%     end;
% end;
...
hdr=cbiReadNiftiHeader_unix2(fname);
